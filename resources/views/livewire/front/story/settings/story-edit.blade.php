<div class="my-6 px-3">
    ...
    <div class="relative mt-2 w-full overflow-hidden">
        @include('livewire.front.story.settings.featured-image-edit')
        @error('featuredImage')
    ...
    <div class="mt-6">
        <x-forms.label class="mb-2 text-base font-semibold">
            {{ isset($schedulePublish) ? __('Publish') : __('Schedule a time to publish') }}</x-forms.label>
        <p class="mb-2 text-xs text-gray-500">
            {{ __('Story scheduling uses a 24-hour time format and is utilizing the UTC timezone.') }}
            (m/d/y h:m)</p>
        <x-datetime-picker
            without-timezone
            placeholder="{{ __('Appointment Date') }}"
            parse-format="YYYY-MM-DD HH:mm"
            wire:model="schedulePublish" />
    </div>
    {{-- Subtitle --}}
    <div class="mt-6">
