<div x-data="{ drawerOpen: false }">
    ...
    <form wire:submit="updateAndSave(Object.fromEntries(new FormData($event.target)))">
        @csrf
        <header
            class="sticky top-0 z-40 flex h-16 w-full items-center justify-between bg-primary_light px-2 transition-colors duration-500 sm:px-4 lg:overflow-y-visible">
            <div class="flex items-center">
                ...
                <div class="ml-0 sm:ml-6">
                    <div class="flex items-center">
                        ...
                        <div class="mx-2 hidden h-5 w-0.5 rotate-12 transform border-l border-gray-500 sm:mx-4 sm:flex">
                        </div>
                        <div class="hidden h-full items-center justify-center text-gray-500 sm:flex">
                            <p class="line-clamp-1">{{ __('Create new story') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex shrink-0 items-center">
                <x-buttons.primary-button
                    ...
                    id="saveButton"
                    class="mr-4">
                    <span wire:loading.remove="" wire:target="updateAndSave">{{ __('Save') }}</span><span
                        wire:loading="" wire:target="updateAndSave">{{ __('Saving...') }}</span>
                </x-buttons.primary-button>
        ...
        <div class="relative mx-auto mt-8 flex max-w-3xl flex-col sm:mt-12">
            <section class="fixed inset-y-0 right-0 z-50 flex max-w-full">
                            ...
                            <div class="space-y-3" x-data="{ activeAccordion: 0 }">
                                ...
                                <div>
                                    ...
                                    <div x-cloak x-show="activeAccordion === 0" x-collapse>
                                        @include('livewire.front.story.settings.story-create')
                                    </div>
                                </div>
                                {{-- SEO Settings --}}
                                <div>
                                ...
                                <div>
                                    ...
                                    <div x-cloak x-show="activeAccordion === 2" x-collapse>
                                        @include('livewire.front.story.settings.audio-file-create')
                                        @error('audioFile')
            ...
            <section class="grow px-3 sm:px-0.5" wire:key="title">
                @error('title')
                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
                <x-forms.textarea-constrained
                    ...
                    limit="160"
                    type="text"
                    placeholder="{{ __('New story title here...') }}"
        ...
        document.addEventListener("DOMContentLoaded", function(event) {
            ...
            saveButton.addEventListener('click', function(e) {
                editor.save().then((outputData) => {
                    @this.set('editorData', outputData)
                });
            });
        });
    </script>
